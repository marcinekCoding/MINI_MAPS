...
\end{table}
\section{Wnioski z analizy systemu i uzasadnienie architektury}

\subsection{Uzasadnienie wyboru architektury mmWave + RFID/NFC}

Zestawiając przegląd dostępnych na rynku systemów Smart Building z naszymi wyliczeniami opartymi na teorii informacji, wybór autorskiej architektury wykorzystującej zanonimizowane radary fal milimetrowych (mmWave) wsparte systemem RFID/NFC stanowi rozwiązanie optymalne. W literaturze przedmiotu oraz rozwiązaniach komercyjnych często promuje się systemy oparte na zaawansowanej analityce obrazu (np. z użyciem sieci YOLO) lub czujnikach środowiskowych takich jak mierniki stężenia CO2. Odrzucamy te metody, ponieważ kamery drastycznie obciążają sieć i budzą uzasadniony opór społeczny związany z wymogami RODO, natomiast czujniki pośrednie charakteryzują się zbyt dużym opóźnieniem w stosunku do dynamicznych przerw między zajęciami. Nasz system dokonuje redukcji złożoności już na poziomie syntaktycznym, gdzie radary sprzętowo zamieniają fizyczną obecność studentów na zwykłą liczbę całkowitą (\texttt{integer}), co gwarantuje najwyższą wiarygodność semantyczną przy minimalnym koszcie przesyłu w sieci ZigBee.

\subsection{Wartość pragmatyczna i redukcja niepewności użytkownika}

Wygenerowane i przefiltrowane w ten sposób dane muszą zostać zaprezentowane odbiorcy w sposób maksymalnie pragmatyczny, aby zrealizować główny cel systemu. Zamiast zmuszać studenta do analizy surowych odczytów z czujników, nasz algorytm przekształca warstwę semantyczną w prosty interfejs wyświetlany na ekranach w lobby, wykorzystujący intuicyjną paletę pięciu stanów (od zielonego do czerwonego i szarego). Jak udowodniliśmy za pomocą wzoru Shannona, takie podejście dostarcza statystycznemu odbiorcy konkretną dawkę informacji wynoszącą średnio 2.06 bita przy każdym spojrzeniu na tablicę. Dzięki temu system skutecznie i błyskawicznie redukuje niepewność użytkownika co do sensu udania się na wyższe piętra, eliminując problem pustych przebiegów i optymalizując logistykę wewnątrz gmachu Wydziału MiNI.
\end{document}
