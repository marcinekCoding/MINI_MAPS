...
Wydział Matematyki i Nauk Informacyjnych PW charakteryzuje się dynamicznym środowiskiem, w którym studenci spędzają znaczną część dnia, również poza godzinami zajęć dydaktycznych. Kluczowym wyzwaniem pragmatycznym jest efektywne wykorzystanie dostępnych przestrzeni socjalnych i naukowych. Obecnie student, chcąc sprawdzić dostępność miejsc w popularnych punktach takich jak „Noga” czy pokój „JetBrains”, musi fizycznie udać się do danej sali. Przy dużej liczbie kondygnacji w gmachu generuje to zbędny ruch w ciągach komunikacyjnych (tzw. „puste przebiegi”) i stratę czasu. System „MINI Info” ma na celu cyfryzację i optymalizację tego procesu, dostarczając wiarygodną informację o zajętości kluczowych pomieszczeń już w głównym punkcie wejścia do budynku (lobby).
...
\end{itemize}
Logika interpretacji opiera się na priorytetyzacji komfortu. Przyjęto, że dla studenta kluczowa jest informacja o dostępnym miejscu siedzącym. Stany informacyjne prezentowane w lobby zmieniają się dynamicznie w zależności od obłożenia:
\begin{itemize}
    ...
    \item $>80\%$ (Prawie pełny): krytyczny poziom zajętości; aktywacja systemu rekomendacji alternatywnych sal.
...
\begin{itemize}
    ...
    \item \textbf{Niskorozdzielcze matryce termiczne (np. technologia Grid-Eye):} Są to sensory podczerwieni zbudowane z niewielkiej siatki detektorów (np. 8x8 pikseli). Urządzenie to nie rejestruje klasycznego obrazu, a jedynie „plamy ciepła”. Rozdzielczość jest celowo na tyle niska, że fizycznie niemożliwe jest zidentyfikowanie twarzy lub sylwetki konkretnego studenta. Pozwala jednak ze stuprocentową pewnością określić, że na konkretnych stanowiskach (krzesłach) znajdują się obiekty emitujące ciepło ludzkiego ciała, co umożliwia precyzyjne liczenie zajętych miejsc.
\end{itemize}
Wdrożenie powyższych rozwiązań gwarantuje najwyższą wiarygodność semantyczną danych wejściowych, zapewniając bezbłędne działanie systemu rekomendacji, przy całkowitym braku ryzyka prawnego i etycznego związanego z przetwarzaniem danych osobowych.
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \addlegendentry{Płynna rzeczywistość (ciągły sygnał)}
...
\subsubsection{Zapychanie sieci a próbkowanie, czyli Nyquist wjeżdża na salę}
...
Zebraliśmy próbne statystyki z naszego radaru z najbardziej obleganych godzin (10:00 - 14:00) dla testowej sali Jet Brains. Wyszło nam, że statusy rozkładają się następująco:
\begin{itemize}
    \item \textbf{Zajęty (Czerwony):} Świeci się przez 60\% czasu (prawdopodobieństwo $p = 0.60$).
    \item \textbf{Wolny (Zielony):} Udaje się go trafić przez 35\% czasu ($p = 0.35$).
    \item \textbf{Sprzątanie (Szary):} Zdarza się bardzo rzadko, raptem przez 5\% czasu ($p = 0.05$).
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        \hline
        Czerwony (Zajęty) & $0.60$ & $\sim 0.74$ \\
        \hline
        Zielony (Wolny) & $0.35$ & $\sim 1.51$ \\
        \hline
...
Jak na dłoni widać, że jak system wypluje szary kafelek „Sprzątanie”, to student dostaje gigantyczny strzał informacyjny (aż 4.32 bita zaskoczenia!), bo to mega rzadka sytuacja. Z kolei czerwony kafelek w godzinach szczytu nikogo aż tak nie dziwi, więc niesie tylko 0.74 bita.

Mając te liczby, z łatwością policzyliśmy średnią niepewność (czyli słynną Entropię $H$) całego naszego wirtualnego pokoju:
\begin{equation}
    H(X) = \sum p(x) \cdot I(x) = (0.60 \cdot 0.74) + (0.35 \cdot 1.51) + (0.05 \cdot 4.32) \approx 1.19 \text{ bit/symbol}
\end{equation}

Maksymalna entropia dla układu 3-stanowego wynosi w teorii około $1.58$ bita. Nasz wynik, czyli 1.19 bita na każde spojrzenie na ekran, to świetny dowód pragmatyczny. Pokazuje to czarno na białym, że nasza wizualizacja zdejmuje ze studentów konkretną porcję niewiadomych, optymalizując ich ruchy po wydziale.
