...
\begin{equation}
    \begin{split}
            H(X) = \sum p(x) \cdot I(x) = (0.35 \cdot 1.51) + (0.30 \cdot 1.74) + (0.20 \cdot 2.32) + \\
            (0.10 \cdot 3.32) + (0.05 \cdot 4.32) \approx 2.06 \text{bita}
    \end{split}
\end{equation}
...
W ujęciu teorii informacji taki model prezentacji jest również ilościowo uzasadniony. Zgodnie z wcześniejszym wyliczeniem entropii Shannona, system dostarcza średnio około 2.06 bita informacji na każdą obserwację tablicy, co przekłada się na realną redukcję niepewności użytkownika. W efekcie student szybciej podejmuje decyzję o wyborze strefy, a liczba ,,pustych przebiegów'' między kondygnacjami zostaje zauważalnie ograniczona.
...
Naszym podstawowym wyjściem jest liczba całkowita reprezentująca liczbę osób aktualnie przebywających w danej strefie. Jest to celowo surowa, liczbowa wartość -- nie uproszczony stan semantyczny. Świadomie rezygnujemy z dostarczania jedynie etykiety „wolne/zajęte”, ponieważ student podejmując decyzję o przemieszczeniu się na wyższe piętro ocenia sytuację bardziej subtelnie. Informacja „4/6 miejsc zajętych” jest znacznie wartościowsza decyzyjnie niż samo „częściowo zajęte” -- pozwala ocenić, czy znajdzie się miejsce przy konkretnym biurku, czy warto w ogóle podejmować wędrówkę. Liczba osób stanowi więc metrykę pierwszego rzędu, z której interfejs użytkownika może samodzielnie wyprowadzić dowolną reprezentację wizualną -- ułamek zajętości, pasek postępu, kolor kafelka.
