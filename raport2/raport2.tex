...
\begin{table}[H]
    ...
    \textbf{Syntaktyczna} & Surowe sygnały z czujników mmWave (radary) oraz logi RFID. Czyste wartości liczbowe (\texttt{int}) bez kontekstu przestrzennego. & Brak (czysty szum informacyjny, niezrozumiały dla studenta w holu). \\ \hline
...
\end{table}
\section{Analiza alternatywnych metod akwizycji i wybór technologii}

\subsection{Analiza alternatywnych metod akwizycji (State-of-the-art)}

W procesie projektowania systemu dokonaliśmy przeglądu literatury oraz dostępnych metod rejestracji sygnałów, oceniając ich przydatność w kontekście określonego modelu użytkowego. Rozważaliśmy trzy główne podejścia do monitorowania zajętości stref na wydziale:
\begin{itemize}
    \item \textbf{Metody wizyjne (Computer Vision):} Wykorzystanie kamer i algorytmów detekcji obiektów (np. YOLO) oferuje najwyższą informatywność sygnału, pozwalając na precyzyjną ocenę jakości i specyfiki danych. Jednakże, z punktu widzenia pragmatyki, rozwiązanie to zostało odrzucone ze względu na restrykcyjne normy prywatności (RODO) oraz ogromną nadmiarowość danych, która wymagałaby skomplikowanych metod kompresji obrazu i dużej przepustowości kanału informacyjnego.
    \item \textbf{Sensory pasywnej podczerwieni (PIR):} Jest to rozwiązanie o najniższym koszcie i najprostszej binarnej reprezentacji kodów. Niestety, sygnał z czujników PIR jest wysoce zaszumiony i obarczony artefaktami -- czujnik przestaje dostarczać informacji, gdy obiekt pozostaje w bezruchu. Z perspektywy teorii informacji, wiarygodność i reprezentatywność takich danych w pokoju cichej nauki byłaby niewystarczająca do poprawnej rekonstrukcji stanu świata rzeczywistego.
    \item \textbf{Analiza pośrednia (sensory CO2 / Wi-Fi):} Rozważaliśmy monitorowanie parametrów środowiskowych jako pośrednich wskaźników obecności ludzi. Metody te charakteryzują się jednak bardzo dużymi opóźnieniami w dostarczaniu informacji, co narusza kryterium aktualności sygnału w modelu czasu rzeczywistego.
\end{itemize}

\subsection{Uzasadnienie wyboru technologii (Synteza wniosków)}

Ostateczny wybór radarów milimetrowych (mmWave) wspieranych przez system RFID wynika z chęci optymalizacji relacji między jakością informacji a ograniczeniami systemu. Radar mmWave dostarcza nam sygnał o średniej złożoności, który jest znacznie bardziej odporny na szumy niż PIR, a jednocześnie nie narusza prywatności tak jak systemy wideo.

Zdecydowaliśmy się na tę technologię, ponieważ pozwala ona na:
\begin{enumerate}
    \item \textbf{Redukcję nadmiarowości na poziomie źródła:} Otrzymujemy gotową informację o liczbie osób (typ \texttt{integer}), co eliminuje potrzebę przesyłania i przetwarzania złożonych strumieni multimedialnych.
    \item \textbf{Wysoką wiarygodność semantyczną:} Fuzja danych z radaru (skanowanie ciągłe) i czytnika RFID (zdarzenia progowe) pozwala na skuteczną weryfikację zgromadzonych zasobów i rozwiązanie problemu odwrotnego przy ograniczonej liczbie pomiarów.
    \item \textbf{Efektywność pragmatyczną:} Wykorzystanie standardu ZigBee i protokołu MQTT zapewnia minimalny koszt energetyczny i transmisyjny, co w teorii informacji przekłada się na optymalne zarządzanie archiwum danych przy zachowaniu wysokiej użyteczności dla studenta.
\end{enumerate}

W ten sposób dokonany przez nas wybór stanowi kompromis pomiędzy technicznymi uwarunkowaniami pomiaru a realnymi potrzebami użytkowników, zapewniając dostarczanie sygnałów o najwyższej informatywności przy minimalnych zniekształceniach.
...
Kluczowym kryterium jakości w naszym systemie jest aktualność informacji, czyli maksymalne dopuszczalne opóźnienie między zmianą stanu fizycznego pomieszczenia a aktualizacją danych na ekranie. Dla naszego zastosowania przyjmujemy jako akceptowalne opóźnienie rzędu jednej minuty -- decyzja studenta o przemieszczeniu się nie wymaga danych aktualizowanych co sekundę, a raczej co kilkanaście do kilkudziesięciu sekund. Drugim kryterium jest spójność danych między dwoma źródłami -- czujnikiem mmWave i czytnikiem RFID. Jak opisano w rozdziale dotyczącym semantyki, algorytm wybiera zawsze wyższą z dwóch wartości, traktując ją jako bezpieczniejsze oszacowanie. Z punktu widzenia metrycznego oznacza to, że system jest zaprojektowany tak, aby raczej zawyżać niż zaniżać obłożenie -- co chroni studenta przed niepotrzebną wędrówką do faktycznie zajętej sali.
