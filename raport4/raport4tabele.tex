...
\begin{table}[H]
    \centering
    \caption{Zestawienie deskryptorów hybrydowych dla wybranych przestrzeni Wydziału MiNI PW (wartości uśrednione z sesji pomiarowej).}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lccccccp{4cm}}
        \toprule
        \textbf{Sala / strefa} & $\mathbf{C_{max}}$ & $\mathbf{\overline{O_i}}$ & $\mathbf{\overline{S_i}}$ & $\mathbf{\overline{D_i}}$ & $\mathbf{L_{Aeq}}$ \textbf{[dB]} & $\mathbf{\Psi_i}$ & \textbf{Uwagi semantyczne} \\
        \midrule
        Sala 329 (Laboratorium) & 30 & 0.40 & 0.50 & 0.45 & 52.0 & 0.60 & Umiarkowane obłożenie, lecz zauważalny szum tła (klawiatury, krótkie konsultacje). \\
        Salka biblioteczna I & 15 & 0.40 & 0.15 & 0.10 & 38.5 & 0.10 & Praca indywidualna. Dominacja ciszy i szumu stacjonarnego HVAC. \\
        Salka biblioteczna II & 15 & 0.80 & 0.65 & 0.35 & 55.0 & 0.75 & Praca projektowa. Wysokie obłożenie z wyraźną dominacją pasma mowy ludzkiej. \\
        Strefa JetBrains & 30 & 0.73 & 0.85 & 0.80 & 68.0 & 0.85 & Strefa relaksu/tranzytowa. Wysoki gwar, ciągła zmiana geometrii sceny (ruch). \\
        \bottomrule
    \end{tabular}
    }
    \label{tab:pomiary}
...
Zestawienie pomiarów w Tabeli 2 empirycznie potwierdza zjawisko "semantycznej pustki" samej liczby osób, na które wskazywano we wczesnej fazie projektu. Zauważmy, że Sala 329 oraz Salka biblioteczna I wykazują identyczne znormalizowane obłożenie ($O_i = 0.40$). Gdyby system opierał się wyłącznie na warstwie syntaktycznej radaru mmWave, traktowałby obie te przestrzenie jako równie atrakcyjne. Jednak dzięki fuzji z torem akustycznym, system bezbłędnie identyfikuje Salkę I jako strefę głębokiej ciszy ($S_i = 0.15$, niska dekoncentracja), podczas gdy Sala 329 charakteryzuje się znacznie wyższą aktywnością akustyczną ($S_i = 0.50$). Dodatkowo, kontrast między Salką I i Salką II udowadnia skuteczność współczynnika $\Psi_i$ – rozpoznanie dominacji pasma mowy pozwala algorytmom odrzucić Salkę II dla profilu "Cicha nauka", mimo że nie jest to głośność uciążliwa w sensie bezwzględnym (55 dB).
...
\begin{table}[H]
    ...
    \begin{tabular}{l l p{6.5cm}}
        ...
        \midrule
        Liczba ukończonych sesji (feedback wysłany) & 45 & Zakończone pełnym cyklem (ankieta $\rightarrow$ wybór $\rightarrow$ ocena). \\
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c}
        ...
        Relax & 12 & 33\% & 67\% & 0.35 \\
        \midrule
        \textbf{Łącznie} & \textbf{45} & \textbf{60\%} & \textbf{40\%} & \textbf{0.23} \\
        \bottomrule
    \end{tabular}
    }
    \label{tab:wybory}
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c}
        ...
        \midrule
        \textbf{Łącznie} & \textbf{45} & \textbf{84\% (38)} & \textbf{16\% (7)} & \textbf{8.1} \\
        \bottomrule
    \end{tabular}
    }
...
Potwierdzono hipotezę o wysokiej wrażliwości tego profilu na charakterystykę widmową sygnału, a nie tylko na jego amplitudę. Rozbieżność między umiarkowanym poziomem ciśnienia akustycznego (np. $L_{Aeq} \approx 45\text{ dB}$) a wysokim współczynnikiem udziału pasma mowy ($\Psi_i > 0.6$) skutkowała gwałtownym spadkiem zadowolenia użytkowników. Dowodzi to, że z perspektywy poznawczej, student silniej reaguje na informacyjność szumu (zrozumiała mowa, szelesty) niż na stacjonarny szum tła (np. głośniejsza klimatyzacja). Wymusza to w przyszłych iteracjach algorytmu zaostrzenie wagi $w_S$ dla parametru $\Psi_i$ w przypadku profili wymagających głębokiego skupienia.

\textbf{2. Zjawisko kompensacji cech (Profil „Praca w grupie”):}
Dla studentów realizujących projekty zespołowe zaobserwowano wyraźną elastyczność względem wektora $\vec{q}$. Użytkownicy ci wykazują wysoką tolerancję na podwyższone wartości deskryptora akustycznego ($S_i \approx 0.7$), pod warunkiem, że wskaźnik obłożenia ($O_i$) pozostaje poniżej progu fizycznego dyskomfortu (zazwyczaj $O_i < 0.6$). Co więcej, w niektórych sesjach wyższy szum otoczenia był oceniany wręcz pozytywnie, pełniąc funkcję maskującą dla własnych rozmów grupy.

\textbf{3. Empiryczna kalibracja systemu (Pragmatyka informacji):}
W ujęciu systemowym, zebrany feedback (zarówno oceny binarne, jak i skala 1-10) przestaje być jedynie dodatkiem interfejsowym (UX), a staje się twardym materiałem walidacyjnym (tzw. \textit{ground truth}). Ujawnione rozbieżności udowadniają, że sztywne wagi $\vec{w}$ zdefiniowane w Tabeli 1 oraz liniowe progi normalizacji (np. $L_{min}, L_{max}$) stanowią jedynie punkt wyjścia. Negatywne sprzężenie zwrotne jest niezbędne do nieliniowej kalibracji indeksu przestrzennego, aby warstwa syntaktyczna (odczyty z czujników) prawidłowo mapowała się na warstwę pragmatyczną (rzeczywistą użyteczność dla studenta).
...
\begin{table}[H]
    ...
    \begin{tabular}{p{9cm} c}
        ...
        $E_{occ}$ (obłożenie / dynamika) & 14\% (1) \\
        \bottomrule
    \end{tabular}
    }
