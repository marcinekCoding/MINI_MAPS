...
Zamiast teoretyzować o działaniu fizycznych czujników, przenieśliśmy logikę bezpośrednio do naszej aplikacji internetowej. Ten rozdział krótko opisuje, jak przechowujemy dane o salach i jak system reaguje na bieżąco na zgłoszenia od użytkowników.
...
Dzięki takiemu formatowi od razu wiemy, gdzie znajduje się sala (\texttt{floor}) oraz ile osób maksymalnie pomieści (\texttt{capacity}). W kodzie wpisaliśmy na stałe $7$ takich przestrzeni (m.in. salki do rozmów 432 i 433, strefę JetBrains, czy odnogę WRS), żeby móc łatwo testować nasz algorytm wyszukiwania.

\subsection{Zastąpienie czujników suwakami}
Ponieważ nasza aplikacja działa w przeglądarce i nie jest bezpośrednio podłączona do sprzętowych mikrofonów czy radarów, użyliśmy suwaków w specjalnym panelu sterowania. Pozwalają one „ręcznie” zmieniać wartości $O$, $S$ i $D$ dla dowolnej sali w czasie rzeczywistym.

Za każdym razem, gdy przesuniemy suwak, wywoływana jest funkcja \texttt{onSensorChange()}. Aplikacja natychmiast aktualizuje dane w pamięci i tłumaczy „suche” ułamki na zrozumiały tekst --- na przykład wartość szumu $0.8$ zmienia się w komunikat \emph{85 dB (Hałas uciążliwy)}. Dzięki temu interfejs jest po prostu przyjazny dla ludzi.
